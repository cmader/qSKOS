...
Binary releases can be downloaded directly from this repository's {download section}[https://github.com/cmader/qSKOS/downloads]. If you cannot or do not want to use {git}[http://git-scm.com/], you can get the packaged source as {zip}[https://github.com/cmader/qSKOS/zipball/master] or {gzipped tar}[https://github.com/cmader/qSKOS/tarball/master] file.
...
* Clone the project (<tt>git clone git@github.com:cmader/qSKOS.git</tt>) or download the source code as described above.
...
<tt>java -jar qSKOS-cmd.jar analyze -c lc,apc ipsv_skos.rdf</tt>

The command above triggers analysis of the "Label Conflicts" and "Ambiguously Preflabeled Concepts" issues. In a very similar way it is possible to explicitly *exclude* issues from testing. For example, the command
...
==== 6) Suppressing informative output
Some issues provide additional informative output about their processing steps which you can observe on the console as those lines containing the text <tt>INFO</tt>. This is useful for interactive use of qSKOS but can be distracting when concise quality issue reports should be generated. The <tt>-q</tt> (quiet) command line switch suppresses output of any additional information at processing time.

The command <tt>java -jar qSKOS-cmd.jar summarize -c ac ipsv_skos.rdf</tt> reports all authoritative conceps in the vocabulary. However, it also produces additional informative output regarding what concept URIs are considered authoritative. Using <tt>java -jar qSKOS-cmd.jar summarize -q -c ac ipsv_skos.rdf</tt> (mind the additional <tt>-q</tt> parameter) only outputs the report results.
