...
Here's an excerpt from the output:
  ...
  ID: alr
  Name: Ambiguously Labeled Resources
...
<tt>java -jar qSKOS-cmd.jar analyze -c lc,alr ipsv_skos.rdf</tt>

The command above triggers analysis of the "Label Conflicts" and "Ambiguously Labeled Resources" issues. In a very similar way it is possible to explicitly *exclude* issues from testing. For example, the command
