...
=== How can I write the result to a file?
...
<tt>java -jar qSKOS-cmd.jar summarize -q -c ac ipsv_skos.rdf > auth_concepts.txt</tt> generates a text file <tt>auth_concepts.txt</tt> containing the result.

A command-line parameter that might be handy when redirecting qSKOS' output into a file is <tt>-np</tt>. It suppresses output of the progress indicator which is thus not written into the generated result.
